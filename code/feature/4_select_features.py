...
def process():
    ...
    #循环多次取平均值
    topN = 1
    topN_thre = int(sys.argv[1])
    pre_acc = 0
    pre_tp = 0
    selected_feature = []
    newDimensions = dimensions[:topN]
    x_train,y_train,x_test,y_test = readXY('train.data','test.data',newDimensions)
    while topN <= topN_thre:
        acc = 0
        iter_num = 5
        for i in range(iter_num):
            ...
            #GBDT算法
            print 'topN: ' + str(topN)
            ret_acc,tp,fp,tn,fn = gbdt(x_train,x_test,y_train,y_test)
            if tp > pre_tp:
                print 'tp: ' + str(tp)
                print newDimensions
                pre_tp = tp
        ...
        avg_acc = acc * 1.0 / iter_num
        if avg_acc >= pre_acc:
            selected_feature.append(newDimensions[-1])
            newDimensions.append(dimensions[topN])
            pre_acc = avg_acc
        else:
            del(newDimensions[-1])
            newDimensions.append(dimensions[topN])
